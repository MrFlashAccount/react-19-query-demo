...
var Mutation = class extends Removable {
  ...
  canSafelyRemove() {
    return this.#observers.length === 0 && this.state.status !== "pending";
  }
  continue() {
...
var MutationCache = class extends Subscribable {
  ...
  performGarbageCollection() {
    for (const mutation of this.#mutations) {
      if (mutation.isEligibleForGc() && mutation.canSafelyRemove()) {
...
var GCManager = class {
  ...
  constructor(config = {}) {
    ...
    this.#maxScanInterval = config.maxScanInterval ?? 3e5;
  ...
  startScanning() {
    ...
    this.#isPaused = false;
    console.log("Running a GC manager!!!");
  ...
  #validateInterval(ms) {
    if (typeof ms !== "number" || !Number.isFinite(ms)) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[GCManager] Invalid scan interval: ${ms}. Using default 30000ms.`);
      }
      return 3e4;
    }
    if (ms < this.#minScanInterval) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[GCManager] Scan interval ${ms}ms is below minimum ${this.#minScanInterval}ms. Using minimum.`);
      }
      return this.#minScanInterval;
    }
    if (ms > this.#maxScanInterval) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[GCManager] Scan interval ${ms}ms exceeds maximum ${this.#maxScanInterval}ms. Using maximum.`);
      }
...
var QueryCache = class extends Subscribable {
  ...
  performGarbageCollection() {
    for (const query of this.#queries.values()) {
      if (query.isEligibleForGc() && query.canSafelyRemove()) {
        this.#queries.delete(query.queryHash);
      }
    }
  }
...
async function getDatabase() {
  const database = await Promise.all([fetch("/movies/1.json").then((res) => res.json()), fetch("/movies/2.json").then((res) => res.json())]).then(([movies1, movies2]) => [...movies1, ...movies2]);
...
async function searchMovies(query, limit = DEFAULT_LIMIT) {
  await Promise.resolve();
  const delay = 300 + Math.random() * 500;
  await new Promise((resolve) => setTimeout(resolve, delay));
  const MOVIE_DATABASE = await getDatabase();
  if (!query.trim()) {
    return MOVIE_DATABASE.slice(0, limit);
  }
  ...
  return MOVIE_DATABASE.filter((movie) => {
    ...
    if (movie.genres.genres.some((genre) => genre.text.toLowerCase().includes(searchTerm))) {
      return true;
    }
    if (movie.plot?.plotText.plainText.toLowerCase().includes(searchTerm)) {
      return true;
    }
    const directorCredit = movie.principalCredits?.find((credit) => credit.category.id === "director");
    if (directorCredit?.credits.some((credit) => credit.name.nameText.text.toLowerCase().includes(searchTerm))) {
      return true;
    }
    return false;
  }).slice(0, limit);
...
async function updateMovieRating(movieId, newRating) {
  await Promise.resolve();
  ...
  movie.ratingsSummary.aggregateRating = Math.min(10, parseFloat((newRating + randomDecimal).toFixed(1)));
  return movie;
}
...
function Settings(t0) {
  ...
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = /* @__PURE__ */ jsx("div", { className: "absolute -top-10 left-[20%] z-50 pointer-events-none", children: /* @__PURE__ */ jsxs("div", { className: "relative", children: [
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = /* @__PURE__ */ jsxs("svg", { className: "w-5 h-5 m-auto", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: [
  ...
  let t5;
  if ($[6] !== t3) {
    t5 = /* @__PURE__ */ jsx("button", { onClick: t3, className: "h-[48px] md:h-[52px] aspect-square px-3 rounded-xl border-2 border-gray-200 hover:border-black focus:outline-none focus:border-black transition-all duration-200 bg-white text-gray-700", "aria-label": "Settings", children: t4 });
    $[6] = t3;
    $[7] = t5;
  } else {
    t5 = $[7];
  ...
  let t6;
  if ($[8] !== handleLimitChange || $[9] !== isOpen || $[10] !== movieLimit || $[11] !== onMovieLimitChange) {
    t6 = isOpen && /* @__PURE__ */ jsxs("div", { className: "absolute top-full left-0 mt-2 bg-white rounded-lg shadow-xl border-2 border-gray-200 p-4 w-[400px] z-50", children: [
  ...
  let t7;
  if ($[13] !== t5 || $[14] !== t6) {
    t7 = /* @__PURE__ */ jsxs("div", { className: "flex flex-none relative", children: [
      t2,
      t5,
      t6
    ] });
...
function SearchBox(t0) {
  const $ = compilerRuntimeExports$1.c(17);
  const {
    movieLimit,
    onMovieLimitChange,
    handleSearchChange,
  ...
  let t1;
  if ($[0] !== movieLimit || $[1] !== onMovieLimitChange) {
    t1 = /* @__PURE__ */ jsx(Settings, { movieLimit, onMovieLimitChange });
    $[0] = movieLimit;
    $[1] = onMovieLimitChange;
    $[2] = t1;
  } else {
    t1 = $[2];
  }
  ...
  if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = /* @__PURE__ */ jsx("div", { className: "absolute inset-y-0 left-0 pl-3 md:pl-5 flex items-center pointer-events-none", children: /* @__PURE__ */ jsx("svg", { className: "w-4 h-4 md:w-5 md:h-5 text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" }) }) });
    $[3] = t2;
  } else {
    t2 = $[3];
  }
  let t3;
  if ($[4] !== handleSearchChange) {
    t3 = /* @__PURE__ */ jsx("input", { type: "text", onChange: (e) => {
      handleSearchChange(e.target.value);
    }, placeholder: "Search by title, director, genre, or tags...", className: "w-full pl-10 pr-4 py-2.5 md:pl-12 md:pr-5 md:py-3 text-sm md:text-base border-2 border-gray-200 rounded-xl focus:outline-none focus:border-black transition-all duration-200 placeholder-gray-400" });
    $[4] = handleSearchChange;
    $[5] = t3;
  } else {
    t3 = $[5];
  }
  let t4;
  if ($[6] !== isPending) {
    t4 = isPending && /* @__PURE__ */ jsx("div", { className: "absolute inset-y-0 right-0 pr-3 md:pr-5 flex items-center", children: /* @__PURE__ */ jsx("div", { className: "animate-spin h-4 w-4 md:h-5 md:w-5 border-2 border-gray-300 border-t-black rounded-full" }) });
    $[6] = isPending;
    $[7] = t4;
  } else {
    t4 = $[7];
  ...
  let t5;
  if ($[8] !== t3 || $[9] !== t4) {
    t5 = /* @__PURE__ */ jsxs("div", { className: "relative flex-1", children: [
      t2,
      t3,
      t4
    ] });
  ...
  let t6;
  if ($[11] !== t1 || $[12] !== t5) {
    t6 = /* @__PURE__ */ jsxs("div", { className: "relative max-w-3xl mx-auto flex items-center gap-3", children: [
      t1,
      t5
    ] });
    $[11] = t1;
  ...
  if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = /* @__PURE__ */ jsx("div", { className: "mt-2 text-center text-xs text-gray-400", children: "Cached for 1 minute after last view" });
    $[14] = t7;
  } else {
    t7 = $[14];
  }
  let t8;
  if ($[15] !== t6) {
    t8 = /* @__PURE__ */ jsxs("div", { className: "w-full max-w-6xl mb-6 md:mb-8", children: [
      t6,
      t7
    ] });
    $[15] = t6;
...
function MovieCard(t0) {
  ...
  if ($[2] !== movie.principalCredits) {
    t2 = movie.principalCredits?.find(_temp$1)?.credits[0]?.name.nameText.text || "Unknown";
...
function LocalTanStackQueryTab(t0) {
  ...
  let t1;
  if ($[0] !== movieLimit || $[1] !== onMovieLimitChange) {
    t1 = /* @__PURE__ */ jsx(QueryClientProvider, { client: queryClient, children: /* @__PURE__ */ jsx(LocalTanStackQueryTabContent, { movieLimit, onMovieLimitChange }) });
    $[0] = movieLimit;
    $[1] = onMovieLimitChange;
    $[2] = t1;
  } else {
...
function LocalTanStackQueryTabContent(t0) {
  const $ = compilerRuntimeExports$1.c(18);
  const {
    movieLimit,
    onMovieLimitChange
  } = t0;
  ...
  let t3;
  if ($[4] !== onMovieLimitChange) {
    t3 = (value_0) => {
      startTransition(() => {
        onMovieLimitChange(value_0);
      });
    };
    $[4] = onMovieLimitChange;
    $[5] = t3;
  } else {
    t3 = $[5];
  }
  const handleMovieLimitChange = t3;
  let t4;
  if ($[6] !== handleMovieLimitChange || $[7] !== isPending || $[8] !== movieLimit) {
    t4 = /* @__PURE__ */ jsx(SearchBox, { movieLimit, onMovieLimitChange: handleMovieLimitChange, handleSearchChange, isPending });
    $[6] = handleMovieLimitChange;
    $[7] = isPending;
    $[8] = movieLimit;
    $[9] = t4;
  } else {
  ...
  let t7;
  if ($[12] !== movies.length || $[13] !== t6) {
    t7 = /* @__PURE__ */ jsx("div", { className: "w-full max-w-6xl", children: /* @__PURE__ */ jsx(MovieList, { moviesAmount: t5, children: t6 }) });
    $[12] = movies.length;
    $[13] = t6;
    $[14] = t7;
  } else {
  ...
  let t8;
  if ($[15] !== t4 || $[16] !== t7) {
    t8 = /* @__PURE__ */ jsxs("div", { className: "flex flex-col items-center min-h-screen px-4 pb-20 md:pb-60", children: [
      t4,
      t7
    ] });
    $[15] = t4;
...
function _temp(movie) {
  return /* @__PURE__ */ jsx(MovieCardLocalTanStack, { movie }, movie.id);
}
function MovieCardLocalTanStack(t0) {
  ...
  const handleStarClick = t5;
  let t6;
  if ($[12] !== handleStarClick || $[13] !== isPending || $[14] !== movie) {
    t6 = /* @__PURE__ */ jsx(MovieCard, { movie, onUpdateRating: handleStarClick, isPending });
    $[12] = handleStarClick;
    $[13] = isPending;
    $[14] = movie;
    $[15] = t6;
