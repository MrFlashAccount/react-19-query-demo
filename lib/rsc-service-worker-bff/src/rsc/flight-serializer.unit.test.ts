...
import { createServerAction, serializeToFlightPayload } from "./flight-serializer";
...
describe("flight serializer branch coverage", () => {
  ...
  it("serializes server references as objects and functions", async () => {
    ...
    const byFunction = await serializeToFlightPayload(fn as unknown as object, manifest);
  ...
  it("serializes root function values and fragment children", async () => {
    const rootFunction = await serializeToFlightPayload((() => "ok") as unknown as object, manifest);
    ...
    const fragment = await serializeToFlightPayload(
      fragmentElement as unknown as object,
      manifest,
    );
    const unsupported = await serializeToFlightPayload(Symbol("x") as unknown as object, manifest);
