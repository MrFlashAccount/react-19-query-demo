...
export const SelectionBrush = React.memo(function SelectionBrush(props: SelectionBrushV2Props) {
  ...
  const preventDragStableCallback = useEvent(preventDrag);
  const onDragStartStableCallback = useEvent(onDragStart);
  const onDragStableCallback = useEvent(onDrag);
  const onDragEndStableCallback = useEvent(onDragEnd);
  const onDragCancelStableCallback = useEvent(onDragCancel);

  const { scheduleRAF, cancelRAF } = useRafThrottle();
  const { scheduleRAF: scheduleRAFScroll, cancelRAF: cancelRAFScroll } = useRafThrottle();

  const startDragging = useEvent(() => {
    setIsDragging(true);
    hasPassedDeadZoneRef.current = true;
  });

  const applyBrushPosition = useEvent((rectangle: geometry.DetailedRectangle) => {
  ...
  const updateBrush = useEvent((rectangle: geometry.DetailedRectangle) => {
    if (!isDragging) {
      startDragging();
    }
