...
export function useStepperState(props: StepperStateProps): UseStepperStateResult {
  ...
  const onStepChangeStableCallback = eventCallbackHooks.useEvent(onStepChange);
  const onCompletedStableCallback = eventCallbackHooks.useEvent(onCompleted);

  const resetStepper = eventCallbackHooks.useEvent(() => {
    privateSetCurrentStep(() => ({ current: defaultStep, direction: "initial" }));
  });

  const setCurrentStep = eventCallbackHooks.useEvent(
  ...
  const isCurrentStep = eventCallbackHooks.useEvent((step: number) => step === currentStep.current);

  const nextStep = eventCallbackHooks.useEvent(() => {
    setCurrentStep((current) => current + 1);
  });
  const previousStep = eventCallbackHooks.useEvent(() => {
    setCurrentStep((current) => current - 1);
  });
