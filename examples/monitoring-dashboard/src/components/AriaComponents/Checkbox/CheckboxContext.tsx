...
export function CheckboxGroupProvider(props: CheckboxGroupProviderProps) {
  ...
  const onChangeStableCallback = useEvent(onChange);

  const addSelected = useEvent((selected: string) => {
  ...
  const removeSelected = useEvent((selected: string) => {
    store.setState((state) => {
      if (!state.selected.has(selected)) {
        return state;
      } else {
  ...
  const toggleSelected = useEvent((selected: string) => {
    store.setState((state) => {
      const nextSelected = new Set(state.selected);
...
export function CheckboxStandaloneProvider(props: CheckboxStandaloneProviderProps) {
  ...
  const onChangeStableCallback = useEvent(() => {
