...
export interface UseTimeoutCallbackOptions {
  ...
  readonly ms: number;
  /**
   * Dependencies for {@link useEvent}.
...
export function useTimeout(params: Pick<UseTimeoutCallbackOptions, "deps" | "ms">) {
   ...
   */
  const getDefaultValue = useEvent(() => {
    return ms === 0;
  });
...
export function useTimeoutAPI(params: Pick<UseTimeoutCallbackOptions, "ms">) {
   ...
   * Starts the timer.
   */
  const startTimer = useEvent(async (signal?: AbortSignal) => {
    stopTimer();
   ...
   * Restarts the timer.
   */
  const restartTimer = useEvent(async (signal?: AbortSignal) => {
    stopTimer();
