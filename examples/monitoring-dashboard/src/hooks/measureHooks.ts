...
export function useMeasure(options: Options = {}): Result {
  ...
  const onResizeStableCallback = useEvent<OnResizeCallback>((nextBounds) => {
    set(nextBounds);

    onResize?.(nextBounds);
  });

  const onInitialMeasureStableCallback = useEvent<OnResizeCallback>((nextBounds) => {
    set(nextBounds);
...
export function useMeasureCallback(options: Options & Required<Pick<Options, "onResize">>) {
  ...
  const onResizeStableCallback = useEvent<OnResizeCallback>(onResize);
  ...
  const measureCallback = useEvent(() => {
