...
async function simulateTick(timestamp: number): Promise<void> {
  ...
  if (metrics.length > 0) {
    ...
    for (const metric of metrics) {
      promises.push(metricsTx.store.put(metric));
    }
    promises.push(metricsTx.done as any);
  ...
  if (logs.length > 0) {
    ...
    for (const log of logs) {
      promises.push(logsTx.store.put(log));
    }
    promises.push(logsTx.done as any);
    await Promise.all(promises);
  }
  ...
  if (serverUpdates.length > 0) {
    ...
    for (const update of serverUpdates) {
      ...
      if (server) {
        ...
        promises.push(serversTx.store.put(server));
      }
    }
    promises.push(serversTx.done as any);
    await Promise.all(promises);
  }
...
async function runSimulation(): Promise<void> {
  ...
  if (lastTime && now - lastTime > 2000) {
    ...
    console.log(`[Simulation Worker] Backfilling ${Math.round(gap / 1000)}s gap`);
...
async function pruneOldData(): Promise<void> {
  ...
  if (metricsDeleted > 0 || logsDeleted > 0) {
    console.log(`[Simulation Worker] Pruned ${metricsDeleted} metrics, ${logsDeleted} logs`);
  }
}
...
self.onmessage = (event: MessageEvent) => {
  ...
  switch (type) {
    ...
    case "status":
      self.postMessage({
        type: "status",
        running: simulationTimer !== null && !isPaused,
      });
      break;
  }
};
